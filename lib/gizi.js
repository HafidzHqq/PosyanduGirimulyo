...
export function calculateAgeInMonths(birthDate) {
  ...
  return {
    months,
    formatted: `${Math.floor(months / 12)} thn ${months % 12} bln`,
  };
}

export function calculateNutritionResult(formData) {
  const age = calculateAgeInMonths(formData.tanggalLahir);
  const imt = formData.berat / (formData.tinggi / 100) ** 2;
  const gender = formData.jenisKelamin === "L" ? "l" : "p";
  ...
  return {
    ...
    wfa: { ...interpretStatus(zScores.wfa, "BB/U"), z: zScores.wfa.toFixed(2) },
    hfa: { ...interpretStatus(zScores.hfa, "TB/U"), z: zScores.hfa.toFixed(2) },
    wfh: { ...interpretStatus(zScores.wfh, "BB/TB"), z: zScores.wfh.toFixed(2) },
    bmifa: { ...interpretStatus(zScores.bmifa, "IMT/U"), z: zScores.bmifa.toFixed(2) },
  };
}
